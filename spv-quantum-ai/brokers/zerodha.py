...
class ZerodhaAdapter(BaseBroker):
    """Placeholder adapter for Zerodha broker integration."""
    ...
    async def login(self, **credentials: Any) -> BrokerResponse:
        self._connected = True
        return BrokerResponse(success=True, broker=self.name, data={"session": "zerodha-session"})
    ...
    async def get_profile(self) -> BrokerResponse:
        return BrokerResponse(success=True, broker=self.name, data={"name": "Zerodha User"})
