...
class DhanAdapter(BaseBroker):
    """Placeholder adapter for Dhan broker integration."""
    ...
    async def login(self, **credentials: Any) -> BrokerResponse:
        self._connected = True
        return BrokerResponse(success=True, broker=self.name, data={"session": "dhan-session"})
    ...
    async def get_profile(self) -> BrokerResponse:
        return BrokerResponse(success=True, broker=self.name, data={"name": "Dhan User"})
