...
class InteractiveBrokersAdapter(BaseBroker):
    """Placeholder adapter for Interactive Brokers integration."""
    ...
    async def login(self, **credentials: Any) -> BrokerResponse:
        self._connected = True
        return BrokerResponse(success=True, broker=self.name, data={"session": "ibkr-session"})
    ...
    async def get_profile(self) -> BrokerResponse:
        return BrokerResponse(success=True, broker=self.name, data={"name": "IBKR User"})
