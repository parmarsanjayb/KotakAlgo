...
class UpstoxAdapter(BaseBroker):
    """Placeholder adapter for Upstox broker integration."""
    ...
    async def login(self, **credentials: Any) -> BrokerResponse:
        self._connected = True
        return BrokerResponse(success=True, broker=self.name, data={"session": "upstox-session"})
    ...
    async def get_profile(self) -> BrokerResponse:
        return BrokerResponse(success=True, broker=self.name, data={"name": "Upstox User"})
